...
output_dir = 'C:/Users/matta/Code/.vscode/BeerwulfForcast/output/model_training/gb_full'
...
def get_param_grid():
    return {
        'rfecv__step': [1],
        'rfecv__min_features_to_select': [3, 10],
...
def evaluate_model(model, X_train, X_test, y_train, y_test, model_name, output_dir):
    ...
    with open(os.path.join(output_dir, 'gb_full_performance.csv'), 'a', newline='') as file:
...
def nested_cross_validation(model, X, y, param_grid):
    ...
    nested_score = cross_val_score(grid_search, X, y, cv=outer_cv, scoring='neg_mean_squared_error')
    logging.info("Finished nested cross-validation")
    return nested_score.mean()

def evaluate_and_log_model(args):
    rfecv_param_set, regressor_params, preprocess_pipeline, X_train, X_test, y_train, y_test, output_dir = args
...
def make_predictions(model, features_mask, preprocess_pipeline, data):
    # Prepare data using the preprocessor and apply the feature mask
    X_data = preprocess_pipeline.transform(data)
    X_selected = X_data[:, features_mask]
...
def main():
    ...
    data, recent_data = load_and_prepare_data(filepaths)
    X, y = setup_data(data)
    preprocess_pipeline = build_preprocessing_pipeline()
    param_grid = get_param_grid()

    # Initialize performance log
    with open(os.path.join(output_dir, 'gb_full_performance.csv'), 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Model', 'Mean Nested CV Score', 'RMSE', 'MAE'])

    # Execute model evaluation across all data in parallel
    ...
    for rfecv_param_set in rfecv_params_list:
        for regressor_params in regressor_params_list:
            args_list.append((rfecv_param_set, regressor_params, preprocess_pipeline, X, X, y, y, output_dir))

    with multiprocessing.Pool() as pool:
        pool.map(evaluate_and_log_model, args_list)
    ...
    # Load model and feature mask for predictions
    loaded_model, features_mask = load_model_and_features(output_dir)
    recent_data_prepared = recent_data.drop(['Sales', 'Month'], axis=1)
    predictions = make_predictions(loaded_model, features_mask, preprocess_pipeline, recent_data_prepared)
    
    # Prepare the DataFrame to store predictions
    recent_data['Predicted_Sales'] = predictions
    predictions_output_file = os.path.join(output_dir, 'predictions.csv')
    recent_data[['Product', 'Month', 'Predicted_Sales']].to_csv(predictions_output_file, index=False)
